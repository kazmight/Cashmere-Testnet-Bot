...
class CryptoBotUI extends EventEmitter {
  constructor(options = {}) {
    ...
    this.delayOverlay = blessed.text({
      ...
      tags: true,
      content: '',
      style: { fg: this.opts.colors.cyan },
      hidden: true
    });
    ...
    this.timerOverlay = blessed.text({
      ...
      tags: true,
      content: '',
      style: { fg: this.opts.colors.secondary },
      hidden: true
    });
  ...
  // Countdown animation in logs (non-spam)
  ...
  // Elapsed timer overlay while waiting on-chain confirmation
  // Returns a stop() function you MUST call when done
